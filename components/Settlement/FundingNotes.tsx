  ...
  return (
    <section aria-labelledby="funding-notes-heading">
          ...
          <div key={`${borrower.id}-${lender.id}`} className="flex items-center justify-between gap-4 px-4 py-3.5 sm:px-5">
            <div className="min-w-0">
              <p className="text-sm text-gray-700">
                <span className="font-medium text-gray-950">{lender.name}</span> advanced cash for <span className="font-medium text-gray-950">{borrower.name}</span>
              </p>
      ...
      <p className="mt-2 text-xs leading-5 text-gray-400">
        Only advances that are still owed are added to the final payment plan; buy-ins, chips, and the pot are unchanged.
      </p>
    </section>
  );
}
