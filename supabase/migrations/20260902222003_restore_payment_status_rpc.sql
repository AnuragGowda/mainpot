-- Repair a production migration-history gap: the deployed client uses the
-- guarded payment-status RPC, while the hosted database predates the function
-- and its post-settlement policy. Keep the phase check in the database so a
-- client cannot record a payment before the host has locked the settlement.
